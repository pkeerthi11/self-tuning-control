# -*- coding: utf-8 -*-
"""
Created on Thu Dec 12 14:49:01 2024
...
if __name__ == '__main__':
    ...
    sigma = 0.01
    change_time = 1500
    control_start_time = 200
    mi = (change_time, 1, 0.5)
    it = [20, 20, 0]
    tau_theta = 5
    prop_theta = 25
    ...
    history_no = sim.single_simulation_striatal(constants, simulation_time, dt, z_controller, init_state=it, mid_increase=mi,
                                       steady_state_pad=steady_state_pad)
    
    history_adaptive = sim.single_simulation_striatal(constants, simulation_time, dt, control_mechanism=a_controller,
                                             control_start=control_start_time, init_state=it, mid_increase=mi,
                                             steady_state_pad=steady_state_pad)
    
    history_proportional = sim.single_simulation_striatal(constants, simulation_time, dt, control_mechanism=p_controller,
                                                 control_start=control_start_time, init_state=it, mid_increase=mi,
                                                 steady_state_pad=steady_state_pad)
    
    history_memoryless = sim.single_simulation_striatal(constants, simulation_time, dt, control_mechanism=m_controller,
                                             control_start=control_start_time, init_state=it, mid_increase=mi,
                                             steady_state_pad=steady_state_pad)
    ...
    #ax[0,0].axvline(control_start_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[0,1].axvline(control_start_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[1,0].axvline(control_start_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[1,1].axvline(control_start_time, color = 'k', linestyle = '--') #, label='Controller On')
    
    ax[0,0].axvline(change_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[0,1].axvline(change_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[1,0].axvline(change_time, color = 'k', linestyle = '--') #, label='Controller On')
    ax[1,1].axvline(change_time, color = 'k', linestyle = '--') #, label='Controller On')
